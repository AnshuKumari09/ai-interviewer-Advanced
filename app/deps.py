...
def set_auth_cookies(response: Response, session) -> None:
    # Frontend (Vercel) aur backend (Render) alag domains par hain, isliye
    # cross-site cookie ke liye samesite="none" + secure=True zaroori hai.
    # "lax" cross-origin fetch/XHR/WebSocket me cookie bhejta hi nahi.
    response.set_cookie(
    ...
    response.set_cookie(
        ...
        httponly=True, samesite="none", secure=True, max_age=60 * 60 * 24 * 7, path="/",
    )
